...
class TdCollapse extends StatefulWidget {
  // 数据
  final List<TdCollapseItem> children;
  // 默认展示
  ...
  @override
  TdCollapseState createState() => TdCollapseState();
}

class TdCollapseState extends State<TdCollapse> {
  // 内容左右padding
  ...
  void initState() {
    ...
    if (widget.defaultActive != null) {
      activeIndex = widget.defaultActive == null ? [] : widget.defaultActive!;
    }
  }
  ...
  void didChangeDependencies() {
    ...
    border =
        Divider(height: 1, color: TdUi.getTheme(context).defaultBorderColor);
  }
  ...
  // 渲染title
  Widget buildTitle(String key, int index, TdCollapseItem item) {
      ...
      titleWidget = Padding(
          ...
          child: Row(children: [
            ...
            Transform.rotate(
                ...
                child: Icon(TdIcons.down, size: 26.0, color: Color(0xff969799)))
          ]));
    }
  ...
  Widget buildItem(String key, int index, TdCollapseItem item) {
...
class TdCollapseItem {
  ...
  TdCollapseItem({this.key, required this.title, required this.child});
}
