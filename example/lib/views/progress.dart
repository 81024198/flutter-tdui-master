  ...
  Widget build(BuildContext context) {
    return Sample('Progress',
        ...
        child: Column(children: <Widget>[
          ...
          TdProgress(value: value),
          TextTitle('自定义样式', noPadding: true),
          TdProgress(
              value: value,
              height: 6.0,
              decoration: BoxDecoration(color: Color(0xffebebeb)),
              higDecoration: BoxDecoration(color: Color(0xff26a2ff))),
          TextTitle('左右自定义内容', noPadding: true),
          TdProgress(
              value: value,
              beforeWidget: Padding(
                  padding: EdgeInsets.only(right: 8.0), child: Text('$value%')),
              afterWidget: Padding(
                  padding: EdgeInsets.only(left: 8.0), child: Text('100%'))),
          TextTitle('Icon', noPadding: true),
          TdProgress(
              value: value,
              afterWidget: Padding(
                  padding: EdgeInsets.only(left: 8.0),
                  child: Icon(TdIcons.close, color: Colors.red))),
          Padding(
              padding: EdgeInsets.all(20.0),
              child: TdButton('上传', onClick: startUpload))
        ]));
  }
}
