...
class IndexState extends State<Index> {
  ...
  // // 二维码
  // void scan() async {
  //   final toast = TdToast.info(context);
  //   try {
  ...
  void toggleTheme() {
    ...
    this.setState(() {
      isDefault = !isDefault;
      TdToast.info(context)('当前为' + (isDefault ? '默认' : '自定义') + '主题');
    });
  }
  ...
  List<Widget> renderSubItem(subList) {
    ...
    subList.forEach((dynamic item) {
      final List<Widget> content = [
        Container(
            ...
            child: Row(children: <Widget>[
              ...
              Container(
                  child:
                      Image.asset('assets/images/right-icon.png', height: 16.0))
            ]))
      ];
      ...
      widgetList.add(InkWell(
          onTap: () {
            if (item['url'] == null) {
              TdToast.info(context)('正在努力开发中...', duration: 1500);
              return;
            }
    ...
    return Opacity(
        ...
        child: Padding(
            padding: EdgeInsets.only(
                top: 25.0, right: listPadding, bottom: 25.0, left: listPadding),
            child: Row(children: <Widget>[
              // 名称
  ...
  Widget build(BuildContext context) {
    ...
    final theme = TdUi.getTheme(context);

    list.forEach((item) {
      children.add(TdCollapseItem(
          title:
              Text(item['title'] as String, style: TextStyle(fontSize: 16.0)),
          child: Column(children: renderSubItem(item['childer']))));
    });
          ...
          Stack(children: [
            Container(
                ...
                child: Column(children: [
                  Row(children: [
                    Text('Flutter TdUi', style: TextStyle(fontSize: 25.0))
                  ]),
                  Container(
